...
1. [프로젝트 구조](#1-프로젝트-구조) : 디렉토리 구조, 파일 구성
2. [요구사항](#2-요구사항) : 하드웨어/소프트웨어 요구사항, 설치 방법
3. [시스템 아키텍처](#3-시스템-아키텍처) : 전체 파이프라인 흐름도
4. [핵심 모듈 구현](#4-핵심-모듈-구현) : StereoCamera, DepthEstimator, DistanceMeter 클래스
5. [GUI 구현](#5-gui-구현) : DistanceMeterGUI (렌더링, 마우스 처리, 정보 표시)
6. [측정 모드](#6-측정-모드) : 단일 포인트, 연속, 다중 포인트, 라인, 영역 측정
7. [정확도 향상 기법](#7-정확도-향상-기법) : 시간/공간 필터링, 신뢰도 가중 측정, 이상치 제거
8. [결과 저장 및 로깅](#8-결과-저장-및-로깅) : CSV/JSON 저장, 스크린샷 관리
9. [전체 코드](#9-전체-코드) : main.py, app_config.yaml
10. [테스트 및 검증](#10-테스트-및-검증) : 정확도 테스트, 성능 벤치마크


📁 포함된 완전한 코드
   * stereo_camera.py - 스테레오 카메라 캡처 및 정류
   * depth_estimator.py - 스테레오 매칭 및 깊이 변환
   * distance_meter.py - 거리 측정 및 포인트 관리
   * gui.py - GUI 렌더링 및 사용자 상호작용
   * measurement_modes.py - 다양한 측정 모드 (단일/다중/라인/영역)
   * accuracy_utils.py - 정확도 향상 유틸리티
   * logging_utils.py - 결과 저장 및 로깅
   * main.py - 메인 애플리케이션
   * app_config.yaml - 설정 파일
...
| 구분 | 기능명 | 설명 |
|----|----|----|
| 1 | 중앙 거리 측정 | 실시간 화면 중앙 픽셀의 거리(Depth)를 실시간으로 표시 |
| 2 | 클릭 포인트 측정 | 마우스 클릭을 통해 특정 위치의 거리 측정 |
| 3 | 다중 포인트 측정 | 여러 개의 측정 포인트를 저장하고 동시에 표시 |
| 4 | 라인 측정 | 두 점을 선택하여 3D 공간상의 실제 거리 계산 |
| 5 | 영역(ROI) 측정 | 선택한 영역에 대해 평균 / 최소 / 최대 깊이 값 계산 |
| 6 | WLS 필터 | WLS(Weighted Least Squares) 필터를 적용하여 시차 맵 품질 향상 |
| 7 | 시간 평균 | 다수 프레임을 평균 처리하여 노이즈 감소 |
| 8 | 결과 저장 | 측정 결과를 CSV, JSON, 스크린샷 형식으로 저장 |
