...
1. [임베디드 스테레오 비전 개요](#1-임베디드-스테레오-비전-개요) : 도전과제, 플랫폼 계층
2. [플랫폼 비교 및 선택](#2-플랫폼-비교-및-선택) : Jetson/STM32 상세 비교, 용도별 추천
3. [NVIDIA Jetson 구현](#3-nvidia-jetson-구현) : 환경 설정, GStreamer, 기본 스테레오
4. [Jetson CUDA 최적화](#4-jetson-cuda-최적화) : CUDA 커널, SAD/Census 매칭, Python 바인딩
5. [Jetson VPI 활용](#5-jetson-vpi-활용) : VPI 스테레오, 하드웨어 가속
6. [STM32H7 구현](#6-stm32h7-구현) : 시스템 개요, CubeMX 설정, 메인 펌웨어
7. [STM32H7 카메라 인터페이스](#7-stm32h7-카메라-인터페이스) : OV7670 드라이버, DCMI 설정
8. [STM32H7 스테레오 매칭](#8-stm32h7-스테레오-매칭) : 최적화된 블록 매칭, SIMD, Census
9. [성능 최적화 기법](#9-성능-최적화-기법) : Jetson/STM32 최적화 체크리스트
10. [실습 프로젝트](#10-실습-프로젝트) : Jetson 완전한 시스템

📁 포함된 코드
   * Jetson:
      * jetson_stereo_basic.py - Jetson 기본 스테레오 구현
      * cuda_stereo.cu - CUDA 커널 (SAD, Census)
      * cuda_stereo_wrapper.py - CUDA Python 래퍼
      * jetson_vpi_stereo.py - VPI 스테레오 시스템
      * jetson_complete_system.py - 완전한 Jetson 시스템
   * STM32H7:
      * main.c - 메인 펌웨어
      * ov7670.h/c - OV7670 카메라 드라이버
      * dcmi.c - DCMI 설정
      * stereo_matching.h/c - 스테레오 매칭 (기본, SIMD, Census)
      * benchmark.c - 성능 벤치마크
