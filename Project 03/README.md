...
## 📋 Project 03: 3D 스캐너 주요 내용

| 섹션 | 내용 |
|------|------|
| **1. 프로젝트 구조** | 디렉토리 구조, 모듈 구성 |
| **2. 시스템 아키텍처** | 6단계 워크플로우 (캡처→생성→정합→후처리→메쉬→출력) |
| **3. 다중 시점 캡처** | ScanSession, ScanSessionManager, CaptureController |
| **4. 포인트 클라우드 생성** | PointCloudGenerator, 배경 제거 (BackgroundRemover) |
| **5. 포인트 클라우드 정합** | ICP, Global Registration, MultiViewRegistration |
| **6. 후처리 및 필터링** | PointCloudProcessor (이상치 제거, 다운샘플링, 법선 추정) |
| **7. 메쉬 생성** | MeshGenerator (Poisson, Ball Pivoting, Alpha Shape) |
| **8. 파일 출력** | FileExporter (PLY, OBJ, STL, GLTF 내보내기) |
| **9. GUI 구현** | Scanner3DGUI (캡처 오버레이, 3D 미리보기) |
| **10. 전체 코드** | main.py, scanner_config.yaml |

## 📁 포함된 완전한 코드

| 파일명 | 설명 |
|--------|------|
| `scan_session.py` | 스캔 세션 관리 (CaptureFrame, ScanSession, ScanSessionManager) |
| `capture_controller.py` | 캡처 제어 (수동/자동/턴테이블 모드) |
| `pointcloud_generator.py` | 스테레오 → 포인트 클라우드 변환 |
| `background_removal.py` | 배경/평면 제거 (깊이 차이, RANSAC) |
| `cloud_registration.py` | 포인트 클라우드 정합 (ICP, FPFH, Global Optimization) |
| `cloud_processor.py` | 후처리 파이프라인 |
| `mesh_generator.py` | 메쉬 생성 및 정리 |
| `file_exporter.py` | 다양한 형식으로 내보내기 |
| `gui.py` | GUI 시각화 |
| `main.py` | 메인 애플리케이션 |
...
| 기능 | 설명 |
|------|------|
| **다중 시점 캡처** | 30° 간격 12회 캡처 (360° 커버) |
| **자동/수동 모드** | 수동(키 입력), 타이머, 턴테이블 연동 |
| **배경 제거** | 깊이 차이, 거리 범위, RANSAC 평면 제거 |
| **ICP 정합** | Point-to-Plane ICP, Colored ICP |
| **글로벌 최적화** | 포즈 그래프 기반 전역 정합 |
| **Poisson 재구성** | 매끄러운 워터타이트 메쉬 생성 |
| **3D 프린팅 출력** | STL 형식, 크기 정보 포함 |
